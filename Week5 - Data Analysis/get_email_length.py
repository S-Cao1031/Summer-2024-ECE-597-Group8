...
def preprocessing(phishing):
    ...
    phishing.loc[:, 'Body'] = phishing['Body'].apply(clean_text)
    phishing.loc[:, 'Subject'] = phishing['Subject'].apply(clean_text)

    phishing['Full_Text'] = phishing['Subject'] + phishing['Body']

    # phishing.to_csv('cleaned_phishing_text.csv', index=False)
    return phishing

def fetch_email_length(phishing):
    phishing = preprocessing(phishing).copy()
    phishing['Word_Count'] = phishing.apply(lambda row: len((row['Subject'] + ' ' + row['Body']).split()), axis=1)
    phishing.to_csv('email_and_length.csv', index=False)
    print(phishing)
    return phishing[['Word_Count']]


def email_length(csv_path):
    phishing = pd.read_csv(csv_path, usecols=['Subject', 'Body'])
    length_info = fetch_email_length(phishing)
    # print(type(length_info))
    return length_info

def main():
    result = email_length('CaptstoneProjectData_2024.csv')
    # print(result)
    return result
