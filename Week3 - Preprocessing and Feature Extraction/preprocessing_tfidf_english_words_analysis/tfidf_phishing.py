...
phishing = pd.read_csv('CaptstoneProjectData_2024.csv', usecols=['Subject', 'Body'])
...
phishing['Body'] = phishing['Body'].str.replace("Notice: This message was sent from outside the University of Victoria email system. Please be cautious with links and sensitive information.", "", regex=False)
#print(phishing.iloc[0])

#remove /n /r
phishing['Body'] = phishing['Body'].str.replace(r'[\r\n]+', ' ', regex=True)
phishing['Subject'] = phishing['Subject'].str.replace(r'[\r\n]+', ' ', regex=True)
#print(phishing.iloc[0])
...
phishing['Body'] = phishing['Body'].apply(replace_email)
phishing['Body'] = phishing['Body'].apply(replace_url)
phishing['Subject'] = phishing['Subject'].apply(replace_email)
phishing['Subject'] = phishing['Subject'].apply(replace_url)
...
phishing['Body'] = phishing['Body'].apply(clean_text).apply(tokenize).apply(remove_stopwords)
phishing['Subject'] = phishing['Subject'].apply(clean_text).apply(tokenize).apply(remove_stopwords)
...
phishing['Body'] = phishing['Body'].apply(lemmatize)
phishing['Subject'] = phishing['Subject'].apply(lemmatize)

phishing.to_csv('cleaned_phishing_text.csv', index=False)


# tf-idf
phishing['Text'] = phishing.apply(lambda row: ' '.join(row['Subject']) + ' ' + ' '.join(row['Body']), axis=1)
