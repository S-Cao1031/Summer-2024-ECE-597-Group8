...
def feature_extract_bow(phishing):
    ...
    vectorizer = CountVectorizer(min_df=0.05, stop_words='english', token_pattern=r'\b[a-zA-Z]+\b')
    bow_matrix = vectorizer.fit_transform(phishing['Text'])

    bow_df = pd.DataFrame(bow_matrix.toarray(), columns=vectorizer.get_feature_names_out())
    # tfidf_df.to_csv('tfidf_phishing_text.csv', index=False)
    # Rename columns to include 'tfidf_' prefix
    bow_df = bow_df.rename(columns=lambda x: 'bow_' + x)
    return bow_df
def phishing_bow(csv_path):
    ...
    # phishing.info()
    bow_df = feature_extract_bow(phishing)
    return bow_df
