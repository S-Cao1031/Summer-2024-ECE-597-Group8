...
def replace_email(input_string, replacement_string=''):
    output_string = re.sub(r'\b[A-Za-z0-9._%+-]+@([A-Za-z0-9.-]+\.[A-Z|a-z]{2,})\b', replacement_string, input_string)
    return output_string

def replace_url(input_string, replacement_string=''):
...
def preprocessing(phishing):
    ...
    # 2463
    # phishing.info()
    # 确保 'Subject' 和 'Body' 列都是字符串
    phishing['Body'] = phishing['Body'].astype(str)
    phishing['Subject'] = phishing['Subject'].astype(str)
...
def feature_extract_word2Vec(phishing):
    ...
    phishing['word2Vec_vector'] = phishing['Text'].apply(lambda x: text_to_vector(x, word2Vec_model))

    word2Vec_df = pd.DataFrame(phishing['word2Vec_vector'].tolist())


    # Rename columns to include 'word2Vec_' prefix
    word2Vec_df = word2Vec_df.rename(columns=lambda x: 'word2Vec_' + str(x))
    return word2Vec_df
def phishing_word2Vec(csv_path):
    ...
    # phishing.info()
    word2Vec_df = feature_extract_word2Vec(phishing)
    return word2Vec_df

def main():
    result = phishing_word2Vec('CaptstoneProjectData_2024.csv')
    print(result)
    result.to_csv('word2Vec_phishing_text.csv', index=False)
    return result
